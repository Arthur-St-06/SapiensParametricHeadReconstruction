...
def save_coefficients_to_folder(folder_name, coeff_dict):
    os.makedirs(folder_name, exist_ok=True)

    # Save each coefficient to a separate file
    for key, value in coeff_dict.items():
        if key in ['shape', 'exp', 'pose']:
            file_path = os.path.join(folder_name, f"{key}.txt")
            np.savetxt(file_path, value.cpu())

    print(f"Coefficients saved to folder: {folder_name}")
...
def main(args):
    ...
    # load test images 
    testdata = datasets.TestData('/home/arthur/ParametricHeadReconstruction/DECA/TestSamples/examples', iscrop=args.iscrop, face_detector=args.detector, sample_step=args.sample_step)
    ...
    for i in tqdm(range(len(testdata))):
        ...
        with torch.no_grad():
            ...
            save_coefficients_to_folder(name, codedict)
